<template>
  ...
  <div v-if="!($browserDetect.isIE && $browserDetect.meta.version < 10)" class="widgetRoot">
    <v-container ma-0 pa-0 fluid fill-height align-content-space-between black>
      <v-row no-gutters align-self="start" no-wrap style="line-height:1 !important">
        <v-col class="cleanslate">
          <ccw-w
      ...
      <v-row no-gutters>
        <v-col>
          <ccw-ticker :size="size" :dark="dark">
...
export default {
  ...
  data: () => ({
    ...
    githubUrl:
      "https://api.github.com/repos/BeautifulTrouble/climate-clock-widget/contents/src/clock.json",
    ready: false,
  ...
  computed: {
    ...
    deadline() {
      let msRemainingAtStartDate =
        (this.startDateCO2Budget / this.tonsPerSecond) * 1000;
    ...
    renewablePercent() {
      ...
      return (
        this.renewStartPct +
        (tElapsed / 1000) * this.renewIncPerSecond
      ).toFixed(this.renewPlaces);
    },
  ...
  methods: {
    setSize() {
      ...
      this.$nextTick(() => {
        let width = document.getElementById(`ccw-container-${this._uid}`)
          .clientWidth;
    ...
    openHomepage() {
      window.location.hostname != "climateclock.world" &&
  ...
  created() {
    ...
    window.addEventListener(
      "resize",
      this.resizeInterval
        ? debounce(this.setSize, resizeInterval)
        : this.setSize
    );
    setInterval(() => {
      this.now = new Date();
    ...
    ready() {
      ...
      this.$nextTick(() => {
        let pb = window
          ...
          ch = document.getElementById(`ccw-container-${this._uid}`)
            .clientHeight;
...
@mixin glow($color) {
  ...
  text-shadow: 0.5px 0.5px 0.1em $color, -0.5px -0.5px 0.1em $color,
    -0.5px 0.5px 0.1em $color, 0.5px -0.5px 0.1em $color;
}
