<template>
  <div class="widgetRoot">
      <v-container ma-0 pa-0 fluid fill-height align-content-space-between black white--text flex-column flex-nowrap>
      <v-row no-gutters style="flex:0 1 auto !important">
        <v-col>Flatten The Climate Curve</v-col>
      </v-row>

      <v-row no-gutters>
        <v-col ref="chartRow">
          <ccw-chart-wrapper id="ccw-chart-wrapper" :size="size">
          <ccw-div>
            <ccw-chart :height="($refs.chartRow && $refs.chartRow.clientHeight) || 100" 
              @newK="k = $event"
              :factorA="parseInt(A)" 
              :factorB="parseInt(B)"
              :size="size"
        ...
        <v-col style="overflow-y:scroll">
            ...
            <ccw-scenario>
              <ccw-div>
                <ccw-radio :class="{'checked':preset == 'bad'}" @click="setPreset('bad')">
...
export default {
  ...
  methods: {
    setSize() {
      ...
      this.$nextTick(() => {
        let width = this.$refs.chartRow.clientWidth;
...
#ccw-chart-wrapper { // Use id to increase specificity over cleanslate
  ...
  ccw-radio {
    ...
    &.checked path:nth-of-type(2), &.checked path:nth-of-type(3) {
      fill: white;
    }
  }
